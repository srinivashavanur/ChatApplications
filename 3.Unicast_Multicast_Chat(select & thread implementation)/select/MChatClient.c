...
*/
					...
					*/
					...
					for (;;)
    					{
                				FD_ZERO(&udpAllset);
                				FD_SET(fileno(stdin), &udpAllset);
                				FD_SET(UDPsocket, &udpAllset);
                				maxfd=max(fileno(stdin), UDPsocket);

                				int nready = select(maxfd+1, &udpAllset, NULL, NULL, NULL);
                				...
                				if(FD_ISSET(UDPsocket,&udpAllset))
                				{
                   					getMessage(UDPsocket);
                				}
                				if(FD_ISSET(fileno(stdin),&udpAllset))
                				{
                   					sendMessage(0, UDPsocket);
                				}
    					}
...
void sendMessage(int inSock,int outSock)
{
		...
		printf("Hit CTRL-D to quit from the chat\n");
				...
				bytes=read(inSock, sendBuf1, MAX_LEN);
				...
				else if(sendto(outSock, sendBuf, sizeof(sendBuf), 0, (SA *) &GroupAddress, sizeof(GroupAddress)) < 0)
				{
