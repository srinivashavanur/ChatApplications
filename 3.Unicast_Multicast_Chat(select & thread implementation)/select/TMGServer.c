...
//int cliSock[MAX_CLi];
//fd_set clientAllSet;
//int mTcpSock, tChatTcpSock, udpSock;
int mcastsock, tchatsock, udpsock;
int tclients = 0;
...
int main(int argc,char *argv[])
{
		...
		{
				FD_ZERO(&cAllSet);
				FD_SET(tcp_sock3, &cAllSet);
				FD_SET(udp_socket_fd, &cAllSet);
				FD_SET(tcp_socket1, &cAllSet);
			
				
				maxfd = tcp_socket1;	
				maxfd = max(maxfd,tcp_sock3);
				maxfd = max(maxfd,udp_socket_fd);
				...
				for (i=0; i < MAX_CLIENTS; i++)
				{
						...
						FD_SET(tsocket[i],&cAllSet);
						maxfd=max(maxfd,tsocket[i]);
				}
		
				nready = select(maxfd+1, &cAllSet, NULL, NULL, NULL);
		...
		if (FD_ISSET(tcp_socket1, &cAllSet))
		{
			  mcasttomcast();
		}
		for (i=0; i < MAX_CLIENTS; i++)
		{
			...
			if(FD_ISSET(tsocket[i], &cAllSet))
				tcptoudp(i);
		} 
		 
		if (FD_ISSET(tcp_sock3, &cAllSet))
		{
			  tcpmain();
		}
		
		if (FD_ISSET(udp_socket_fd, &cAllSet))
			udptotcp(udp_socket_fd);	
	...
	void tcptoudp(int tsockref)
	{
						...
						sn = sendto(udp_socket_fd, buf, sizeof(buf), 0, (SA *) &udp_server, sizeof(udp_server));
						if(sn < 0)
						{
	...
	void udptotcp(int inSock)
	{
		...
		//printf("UDPtoTCP (pid: %d)\n",getpid());
						...
						cc1=recv(inSock,recvBuf,MAX_LEN,0);
						if (cc1 < 0) 
						{
