...

{
  ...
  if (sd<0) {
	...
	for(i=0;i<sizeof(server_machines);i++)
	{
		...
		//if ( sendto(sd, buf, sizeof(buf), 0,(SA *) &server, sizeof(server)) > 0 )
		//	break;
	}
  ...
  printf("Connected to MUST Server: ");
   printf("%s\n", hp->h_name);
  printf("%s:%d\n", hp->h_name,ntohs(server.sin_port));
...
/** get data from USER, send it SERVER,
  ...
  for(;;) {
    cleanup(rbuf);
    if( (rc=read(sd, rbuf, sizeof(rbuf))) < 0){
...
for(;;) {
   printf("\nType anything followed by RETURN, or type CTRL-D to exit\n");
    ...
    if (sendto(sd, sendBuf, sizeof(sendBuf), 0,(SA *) &server, sizeof(server)) <0 )
